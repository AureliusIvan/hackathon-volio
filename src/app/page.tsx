...
export default function CameraView() {
  ...
  const switchMode = useCallback(async (newMode: AppMode) => {
    ...
    if (newMode === 'narration') {
      await speakWithSettings('Narration Mode is on now. Tap for details, hold to talk to AI.');
      setFocusTimer(0);
    } else {
      await speakWithSettings('Guidance Mode is on now');
      startGuidanceMode();
    }
  ...
  }, [currentMode, isLoading, speakWithSettings]);
  ...
  const endHoldToTalk = useCallback(async () => {
    ...
    if (holdDuration < 0.5) {
      ...
      // TODO: Implement AI conversation functionality
      await speakWithSettings('AI conversation feature coming soon. For now, I\'ll analyze what I see.');
      await handleNarrationCapture();
    }
  }, [isHoldingToTalk, startFocusTimer, speakWithSettings, handleNarrationCapture]);
      ...
      try {
        ...
        if (video) {
          ...
          video.onloadedmetadata = async () => {
            ...
            await speakWithSettings('Camera ready. Narration Mode is on now. Tap top half for detailed descriptions, bottom half for navigation guidance.');
          };
        }
      } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className={`absolute top-0 left-0 right-0 h-1/2 border-2 ${
          currentMode === 'narration' ? 'border-blue-500 bg-blue-500' : 'border-blue-300 bg-blue-300'
        } bg-opacity-10`}>
          <div className="absolute top-4 left-4 right-4">
            <div className={`text-center p-2 rounded-lg ${
              currentMode === 'narration' ? 'bg-blue-600' : 'bg-blue-400'
              ...
              {isHoldingToTalk && (
                <p className="text-xs mt-1 text-yellow-300">🎤 Holding to talk ({holdTimer.toFixed(1)}s)</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className={`absolute bottom-0 left-0 right-0 h-1/2 border-2 ${
          currentMode === 'guidance' ? 'border-green-500 bg-green-500' : 'border-green-300 bg-green-300'
        } bg-opacity-10`}>
          <div className="absolute bottom-4 left-4 right-4">
            <div className={`text-center p-2 rounded-lg ${
              currentMode === 'guidance' ? 'bg-green-600' : 'bg-green-400'
            } bg-opacity-80 text-white`}>
      ...
      {currentDescription && !isLoading && (
        <div className={`absolute top-1/2 left-4 right-4 transform -translate-y-1/2 ${
          currentDescription.mode === 'narration' ? 'bg-blue-900' : 'bg-green-900'
          ...
          <div className="space-y-3">
            {descriptionHistory.map((desc, index) => (
              <div key={index} className={`p-3 rounded border ${
                desc.mode === 'narration' ? 'bg-blue-800 border-blue-600' : 'bg-green-800 border-green-600'
              } bg-opacity-60`}>
